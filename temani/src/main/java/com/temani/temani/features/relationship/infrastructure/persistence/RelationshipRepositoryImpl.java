...
public class RelationshipRepositoryImpl implements RelationshipRepository {
	...
	public List<Relationship> findAcceptedByUserId(UUID userId) {
		return jpa.findAcceptedByUserId(userId).stream().map(mapper::toDomain).toList();
	}

	@Override
	public List<Relationship> findPendingSentByUserId(UUID userId) {
		return jpa.findAllByInitiatorIdAndAccepted(userId, false).stream().map(mapper::toDomain).toList();
	}

	@Override
	public List<Relationship> findPendingReceivedByUserId(UUID userId) {
		return jpa.findPendingReceivedByUserId(userId).stream().map(mapper::toDomain).toList();
	}
